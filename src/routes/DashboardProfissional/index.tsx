  ...
  return (
    <div className="w-full">
      ...
      <div className="grid w-full grid-cols-3 bg-gray-100  p-1 rounded-lg">
        <button onClick={() => setActiveTab('reports')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'reports' ? 'bg-white  text-gray-900 ' : 'text-gray-500 hover:text-gray-900'}`}>
          Relatórios
        </button>
        <button onClick={() => setActiveTab('addPatient')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'addPatient' ? 'bg-white  text-gray-900 ' : 'text-gray-500 hover:text-gray-900'}`}>
          Cadastrar Paciente
        </button>
        <button onClick={() => setActiveTab('settings')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'settings' ? 'bg-white  text-gray-900 ' : 'text-gray-500 hover:text-gray-900'}`}>
          Configurações
        </button>
      </div>
            ...
            <div className="p-6 pt-0">
              <form onSubmit={handleFormSubmit} className="space-y-8">
                <div className="border-t pt-6">
                  <h3 className="text-lg font-semibold mb-4 text-gray-800">Acessibilidade e Necessidades Especiais</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                    ...
                    <div className="space-y-2">
                      ...
                      <select value={newPatient.digitalSkills || ""} onChange={(e) => setNewPatient({ ...newPatient, digitalSkills: e.target.value })} className="h-10 w-full rounded-md border border-gray-300 bg-white  px-3 py-2 text-sm"><option value="" disabled>Selecione o nível</option><option value="sozinho">Usa celular sozinho</option><option value="ajuda">Precisa de ajuda</option><option value="nao-utiliza">Não utiliza</option></select>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <select value={newPatient.reminderChannel || ""} onChange={(e) => setNewPatient({ ...newPatient, reminderChannel: e.target.value })} className="h-10 w-full rounded-md border border-gray-300 bg-white  px-3 py-2 text-sm"><option value="" disabled>Selecione o canal</option><option value="whatsapp">WhatsApp</option><option value="sms">SMS</option><option value="ligacao">Ligação</option><option value="email">E-mail</option></select>
                    </div>
                  ...
                  <div className="space-y-2 mt-6">
                    <label className="text-sm font-medium text-gray-700">Necessidades de acessibilidade</label>
                    <select value={newPatient.accessibilityNeeds || ""} onChange={(e) => setNewPatient({ ...newPatient, accessibilityNeeds: e.target.value })} className="h-10 w-full rounded-md border border-gray-300 bg-white  px-3 py-2 text-sm"><option value="" disabled>Selecione as necessidades</option><option value="nenhuma">Nenhuma necessidade especial</option><option value="narracao">Narração em áudio</option><option value="fonte-ampliada">Fonte ampliada</option><option value="contraste-alto">Contraste alto</option><option value="teclado-voz">Teclado/voz</option><option value="multiplas">Múltiplas necessidades</option></select>
                  </div>
                </div>
                ...
                <div className="flex gap-4 pt-6 border-t">
                  ...
                  <button type="button" onClick={handleFormClear} className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium border border-gray-300 rounded-md hover:bg-gray-100 transition-colors">
                    Limpar Formulário
                  </button>
                </div>
              </form>
                ...
                <form className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <input value={mockProfessional.name} readOnly className="h-10 w-full rounded-md border border-gray-300 bg-gray-100  px-3 py-2 text-sm" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-700">CRM</label>
                      <input value={mockProfessional.crm} readOnly className="h-10 w-full rounded-md border border-gray-300 bg-gray-100  px-3 py-2 text-sm" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-700">Email</label>
                      <input value={mockProfessional.email} readOnly className="h-10 w-full rounded-md border border-gray-300 bg-gray-100  px-3 py-2 text-sm" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-700">Especialidade</label>
                      <input value={mockProfessional.specialty} readOnly className="h-10 w-full rounded-md border border-gray-300 bg-gray-100  px-3 py-2 text-sm" />
                    </div>
                  </div>
